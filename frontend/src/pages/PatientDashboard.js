      ...
      try {
        ...
        setDashboardData({
          overview: {
            ...
            health: {
              lastCheckup: appointments.find(a => a.status === 'completed')?.date || '',
              nextAppointment: appointments.find(a => a.status === 'scheduled')?.date || '',
              healthScore: 85 // Placeholder - would come from health API
            },
            notifications: activities.filter(a => a.status === 'unread').length
          },
          ...
          upcomingAppointments: appointments
            ...
            .map(app => ({
              id: app._id,
              doctor: app.doctor?.name || 'Doctor',
              specialty: app.doctor?.specialization || 'General',
              date: app.date,
              time: app.time,
              location: app.doctor?.hospital || 'Clinic'
            }))
        });
      } catch (error) {
